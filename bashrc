...
if [[ -f /etc/os-release ]]; then
    ...
    if [ "$ID" == "ubuntu" -a "$(id -u)" -eq 0 ]; then
        PS1='${debian_chroot:+($debian_chroot)}\[\e[1;31m\][\u@\h: \W]\$\[\e[0m\] '
        alias rm='rm -i'
        alias cp='cp -i'
        alias mv='mv -i'
    fi
...
source $HOME/custom/etc/bash_completion_tmux.sh
source $HOME/custom/etc/tmuxinator.bash
...
export POWERLINE_PROMPT="user_info hostname cwd scm python_venv battery"
